...
def load_models(device, cache_dir):
    ...
    # Use context manager to ensure proper memory cleanup
    with torch.amp.autocast() if torch.cuda.is_available() else nullcontext():
...
class OCRPipeline:
    ...
    def process_image(self, image):
        ...
        # Collect all regions for batch classification
        status_text.text("Preparing regions for classification...")
        cropped_regions = []
        preprocessed_inputs = []
        ...
        for box in sorted_boxes:
            ...
            preprocessed_inputs.append(updated_preprocess_image(cropped_region))
        
        # Batch classify all regions
        status_text.text("Classifying text regions (batch processing)...")
        batch_size = 8  # Adjust based on your GPU memory
        
        try:
            # Determine optimal batch size for your GPU
            if torch.cuda.is_available():
                if torch.cuda.get_device_properties(0).total_memory < 6 * 1024 * 1024 * 1024:  # Less than 6GB
                    batch_size = 4
            
            batch_classifications = self.resnet_classifier.classify_batch(
                preprocessed_inputs, 
            ...
            for i, ((class_id, confidence), box) in enumerate(zip(batch_classifications, sorted_boxes)):
                x1, y1, x2, y2 = map(int, box)
                classification_results.append((x1, y1, x2, y2, class_id, confidence))
                
                if class_id == 0:  # Valid text (not strike-through)
                    valid_images.append(cropped_regions[i])
                    valid_indices.append(i)
            
            # Batch process OCR for all valid regions
            status_text.text(f"Performing OCR on {len(valid_images)} valid regions (batch processing)...")
            if valid_images:
                ocr_batch_size = 4  # Smaller batch for the more memory-intensive OCR
                batch_results = self.trocr_ocr.recognize_batch(valid_images, batch_size=ocr_batch_size)
            ...
            # If batch processing fails completely, try legacy method
            # This part is a fallback to the original implementation
